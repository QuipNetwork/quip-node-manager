...
- **Stop halts containers instead of removing them**: stopping the stack now runs `docker compose stop` (containers are stopped and kept) rather than `docker compose down` plus a container reap. The next Start recreates the containers from a clean slate (`down` + name reap moved to the Start path), so name-conflict cleanup and reaping of stale containers from older stack versions still happen — just at Start, not Stop. Named volumes were already preserved.
