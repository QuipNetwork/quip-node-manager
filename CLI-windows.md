# Running a Quip Node via Docker (PowerShell)

## 1. Create the data directory

```powershell
New-Item -ItemType Directory -Force -Path "$HOME\quip-data"
```

## 2. Create `%USERPROFILE%\quip-data\config.toml`
...
> **Note:** Paths in the config must use `/data/...` (the Docker mount point), not local Windows paths.
...
```powershell
New-NetFirewallRule -DisplayName "Quip Node UDP" -Direction Inbound -Protocol UDP -LocalPort 20049 -Action Allow
New-NetFirewallRule -DisplayName "Quip Node TCP" -Direction Inbound -Protocol TCP -LocalPort 20049 -Action Allow
```
...
docker logs -f --tail 100 quip-node
```
